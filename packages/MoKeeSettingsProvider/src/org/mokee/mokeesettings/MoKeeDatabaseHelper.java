...
/**
 * The MoKeeDatabaseHelper allows creation of a database to store MK specific settings for a user
 * in System, Secure, and Global tables.
 */
public class MoKeeDatabaseHelper extends SQLiteOpenHelper{
    private static final String TAG = "MoKeeDatabaseHelper";
    ...
    /**
     * Creates an instance of {@link MoKeeDatabaseHelper}
     * @param context
     * @param userId
     */
    public MoKeeDatabaseHelper(Context context, int userId) {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (upgradeVersion < 3) {
            ...
            try {
                stmt = db.compileStatement("INSERT INTO secure(name,value)"
                        + " VALUES(?,?);");
                loadStringSetting(stmt, MoKeeSettings.Secure.PROTECTED_COMPONENT_MANAGERS,
        ...
        if (upgradeVersion < 4) {
            /* Was set MoKeeSettings.Secure.MK_SETUP_WIZARD_COMPLETE
        ...
        if (upgradeVersion < 7) {
            if (mUserHandle == UserHandle.USER_OWNER) {
                ...
                try {
                    ...
                    stmt.bindString(1, MoKeeSettings.System.STATUS_BAR_CLOCK);
                    long value = stmt.simpleQueryForLong();

                    if (value != 0) {
                        stmt = db.compileStatement("UPDATE system SET value=? WHERE name=?");
                        stmt.bindLong(1, value - 1);
                        stmt.bindString(2, MoKeeSettings.System.STATUS_BAR_CLOCK);
                        stmt.execute();
                    }
                    db.setTransactionSuccessful();
                } catch (SQLiteDoneException ex) {
                    // MoKeeSettings.System.STATUS_BAR_CLOCK is not set
                } finally {
                    if (stmt != null) stmt.close();
                    db.endTransaction();
                }
        ...
        if (upgradeVersion < 8) {
            ...
            try {
                ...
                stmt.bindString(2, MoKeeSettings.Secure.PROTECTED_COMPONENT_MANAGERS);
                stmt.execute();
                db.setTransactionSuccessful();
            } finally {
                if (stmt != null) stmt.close();
        ...
        if (upgradeVersion < 10) {
            if (mUserHandle == UserHandle.USER_OWNER) {
                ...
                try {
                    ...
                    stmt.bindString(1, MoKeeSettings.Secure.MK_SETUP_WIZARD_COMPLETED);
                    stmt.execute();
                    db.setTransactionSuccessful();
                } finally {
                    if (stmt != null) stmt.close();
        ...
        if (upgradeVersion < 11) {
            ...
            if (mUserHandle == UserHandle.USER_OWNER) {
                ...
                try {
                    ...
                    stmt.bindString(1, MoKeeSettings.Global.DEV_FORCE_SHOW_NAVBAR);
                    long value = stmt.simpleQueryForLong();

                    stmt = db.compileStatement("INSERT INTO system (name, value) VALUES (?, ?)");
                    stmt.bindString(1, MoKeeSettings.System.FORCE_SHOW_NAVBAR);
                    ...
                    // MoKeeSettings.Global.DEV_FORCE_SHOW_NAVBAR is not set
                } finally {
                    if (stmt != null) stmt.close();
                    db.endTransaction();
                }
        ...
        if (upgradeVersion < 12) {
            if (mUserHandle == UserHandle.USER_OWNER) {
                ...
                try {
                    stmt = db.compileStatement("SELECT value FROM system WHERE name=?");
                    stmt.bindString(1, MoKeeSettings.System.STATUS_BAR_BATTERY_STYLE);
                    long value = stmt.simpleQueryForLong();
                    ...
                    stmt.bindString(2, MoKeeSettings.System.STATUS_BAR_BATTERY_STYLE);
                    stmt.execute();
                    db.setTransactionSuccessful();
                } catch (SQLiteDoneException ex) {
                    // MoKeeSettings.System.STATUS_BAR_BATTERY_STYLE is not set
                } finally {
                    if (stmt != null) stmt.close();
                    db.endTransaction();
                }
    ...
    private void loadSecureSettings(SQLiteDatabase db) {
        ...
        try {
            stmt = db.compileStatement("INSERT OR IGNORE INTO secure(name,value)"
                    + " VALUES(?,?);");
            // Secure
            loadBooleanSetting(stmt, MoKeeSettings.Secure.ADVANCED_MODE,
    ...
    private void loadSystemSettings(SQLiteDatabase db) {
        ...
        try {
            stmt = db.compileStatement("INSERT OR IGNORE INTO system(name,value)"
                    + " VALUES(?,?);");
            // System
            loadIntegerSetting(stmt, MoKeeSettings.System.FORCE_SHOW_NAVBAR,
                    R.integer.def_force_show_navbar);

            loadIntegerSetting(stmt, MoKeeSettings.System.STATUS_BAR_QUICK_QS_PULLDOWN,
                    R.integer.def_qs_quick_pulldown);

            loadIntegerSetting(stmt, MoKeeSettings.System.BATTERY_LIGHT_BRIGHTNESS_LEVEL,
                    R.integer.def_battery_brightness_level);

            loadIntegerSetting(stmt, MoKeeSettings.System.BATTERY_LIGHT_BRIGHTNESS_LEVEL_ZEN,
                    R.integer.def_battery_brightness_level_zen);

            loadIntegerSetting(stmt, MoKeeSettings.System.NOTIFICATION_LIGHT_BRIGHTNESS_LEVEL,
                    R.integer.def_notification_brightness_level);

            loadIntegerSetting(stmt, MoKeeSettings.System.NOTIFICATION_LIGHT_BRIGHTNESS_LEVEL_ZEN,
                    R.integer.def_notification_brightness_level_zen);

            loadBooleanSetting(stmt, MoKeeSettings.System.SYSTEM_PROFILES_ENABLED,
                    R.bool.def_profiles_enabled);

            loadIntegerSetting(stmt, MoKeeSettings.System.ENABLE_FORWARD_LOOKUP,
                    R.integer.def_forward_lookup);

            loadIntegerSetting(stmt, MoKeeSettings.System.ENABLE_PEOPLE_LOOKUP,
                    R.integer.def_people_lookup);

            loadIntegerSetting(stmt, MoKeeSettings.System.ENABLE_REVERSE_LOOKUP,
                    R.integer.def_reverse_lookup);

            loadBooleanSetting(stmt, MoKeeSettings.System.NOTIFICATION_LIGHT_PULSE_CUSTOM_ENABLE,
                    R.bool.def_notification_pulse_custom_enable);

            loadBooleanSetting(stmt, MoKeeSettings.System.SWAP_VOLUME_KEYS_ON_ROTATION,
                    R.bool.def_swap_volume_keys_on_rotation);

            loadIntegerSetting(stmt, MoKeeSettings.System.STATUS_BAR_BATTERY_STYLE,
                    R.integer.def_battery_style);

            loadIntegerSetting(stmt, MoKeeSettings.System.STATUS_BAR_CLOCK,
                    R.integer.def_clock_position);

            if (mContext.getResources().getBoolean(R.bool.def_notification_pulse_custom_enable)) {
                loadStringSetting(stmt, MoKeeSettings.System.NOTIFICATION_LIGHT_PULSE_CUSTOM_VALUES,
