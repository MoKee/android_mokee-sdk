...
import vendor.mokee.livedisplay.V2_0.IColorBalance;
...
/**
 * Manages access to MK hardware extensions
 *
 *  <p>
 *  This manager requires the HARDWARE_ABSTRACTION_ACCESS permission.
 *  <p>
 *  To get the instance of this class, utilize MoKeeHardwareManager#getInstance(Context context)
 */
public final class MoKeeHardwareManager {
    private static final String TAG = "MoKeeHardwareManager";
    ...
    private static IMoKeeHardwareService sService;
    private static MoKeeHardwareManager sMoKeeHardwareManagerInstance;
     ...
     */
    private MoKeeHardwareManager(Context context) {
        ...
        if (context.getPackageManager().hasSystemFeature(
                MoKeeContextConstants.Features.HARDWARE_ABSTRACTION) && !checkService()) {
            Log.wtf(TAG, "Unable to get MoKeeHardwareService. The service either" +
    ...
    /**
     * Get or create an instance of the {@link mokee.hardware.MoKeeHardwareManager}
     * @param context
     * @return {@link MoKeeHardwareManager}
     */
    public static MoKeeHardwareManager getInstance(Context context) {
        if (sMoKeeHardwareManagerInstance == null) {
            sMoKeeHardwareManagerInstance = new MoKeeHardwareManager(context);
        }
        return sMoKeeHardwareManagerInstance;
    }

    /** @hide */
    public static IMoKeeHardwareService getService() {
        if (sService != null) {
            return sService;
        }
        IBinder b = ServiceManager.getService(MoKeeContextConstants.MK_HARDWARE_SERVICE);
        if (b != null) {
            sService = IMoKeeHardwareService.Stub.asInterface(b);
            return sService;
        }
        return null;
    }
