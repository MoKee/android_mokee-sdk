 ...
 */
public class MoKeeWeatherManager {

    private static IMoKeeWeatherManager sWeatherManagerService;
    private static MoKeeWeatherManager sInstance;
    ...
    private static final String TAG = MoKeeWeatherManager.class.getSimpleName();
    ...
    private MoKeeWeatherManager(Context context) {
        Context appContext = context.getApplicationContext();
        mContext = (appContext != null) ? appContext : context;
        sWeatherManagerService = getService();

        if (context.getPackageManager().hasSystemFeature(
                MoKeeContextConstants.Features.WEATHER_SERVICES) && (sWeatherManagerService == null)) {
            Log.wtf(TAG, "Unable to bind the MoKeeWeatherManagerService");
        }
        mHandler = new Handler(appContext.getMainLooper());
    }

    /**
     * Gets or creates an instance of the {@link mokee.weather.MoKeeWeatherManager}
     * @param context
     * @return {@link MoKeeWeatherManager}
     */
    public static MoKeeWeatherManager getInstance(Context context) {
        if (sInstance == null) {
            sInstance = new MoKeeWeatherManager(context);
        }
        return sInstance;
    }

    /** @hide */
    public static IMoKeeWeatherManager getService() {
        if (sWeatherManagerService != null) {
            return sWeatherManagerService;
        }
        IBinder binder = ServiceManager.getService(MoKeeContextConstants.MK_WEATHER_SERVICE);
        if (binder != null) {
            sWeatherManagerService = IMoKeeWeatherManager.Stub.asInterface(binder);
            return sWeatherManagerService;
        }
        return null;
    }
    ...
    public int requestWeatherUpdate(@NonNull Location location,
        ...
        try {
            int tempUnit = MoKeeSettings.Global.getInt(mContext.getContentResolver(),
