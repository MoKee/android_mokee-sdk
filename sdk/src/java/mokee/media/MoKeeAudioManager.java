 ...
 * {@link mokee.permission.OBSERVE_AUDIO_SESSIONS}
 ...
 * @hide
 */
public final class MoKeeAudioManager {

    private static final String TAG = "MoKeeAudioManager";

    /**
     * Broadcast sent when audio session are added and removed.
     */
    public static final String ACTION_AUDIO_SESSIONS_CHANGED =
            "mokee.intent.action.ACTION_AUDIO_SESSIONS_CHANGED";
    ...
    private static MoKeeAudioManager sInstance;
    private static IMoKeeAudioService sService;

    /**
     * @hide to prevent subclassing from outside of the framework
     */
    private MoKeeAudioManager(Context context) {
        ...
        if (!context.getPackageManager().hasSystemFeature(
                MoKeeContextConstants.Features.AUDIO) || !checkService()) {
            Log.wtf(TAG, "Unable to get MoKeeAudioService. The service either" +
    ...
    /**
     * Get or create an instance of the {@link mokee.media.MoKeeAudioManager}
     * @param context
     * @return {@link MoKeeAudioManager}
     */
    public synchronized static MoKeeAudioManager getInstance(Context context) {
        if (sInstance == null) {
            sInstance = new MoKeeAudioManager(context);
        }
        return sInstance;
    }

    /** @hide */
    public static IMoKeeAudioService getService() {
        if (sService != null) {
            return sService;
        }
        IBinder b = ServiceManager.getService(MoKeeContextConstants.MK_AUDIO_SERVICE);
        if (b != null) {
            sService = IMoKeeAudioService.Stub.asInterface(b);
            return sService;
        }
        return null;
    }
    ...
    private boolean checkService() {
        if (sService == null) {
            Log.w(TAG, "not connected to MoKeeAudioService");
            return false;
        }
        return true;
    }
