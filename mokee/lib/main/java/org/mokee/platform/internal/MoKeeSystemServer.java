...
import org.mokee.platform.internal.common.MoKeeSystemServiceHelper;
 ...
 */
public class MoKeeSystemServer {
    private static final String TAG = "MoKeeSystemServer";
    private Context mSystemContext;
    private MoKeeSystemServiceHelper mSystemServiceHelper;

    private static final String ENCRYPTING_STATE = "trigger_restart_min_framework";
    private static final String ENCRYPTED_STATE = "1";

    public MoKeeSystemServer(Context systemContext) {
        mSystemContext = systemContext;
        mSystemServiceHelper = new MoKeeSystemServiceHelper(mSystemContext);
    }
    ...
    private void startServices() {
        ...
        for (String service : externalServices) {
            try {
                ...
                MoKeeSystemService mokeeSystemService =  mSystemServiceHelper.getServiceFor(service);
                if (context.getPackageManager().hasSystemFeature(
                        mokeeSystemService.getFeatureDeclaration())) {
                    if (coreAppsOnly() && !mokeeSystemService.isCoreService()) {
                        Slog.d(TAG, "Not starting " + service +
                                " - only parsing core apps");
                    } else {
                        Slog.i(TAG, "Starting service " + service);
                        ssm.startService(mokeeSystemService.getClass());
                    }
                } else {
