...
import mokee.weather.IMoKeeWeatherManager;
...
public class MoKeeWeatherManagerService extends MoKeeSystemService {

    private static final String TAG = MoKeeWeatherManagerService.class.getSimpleName();
            ...
            synchronized (mMutex) {
                ...
                switch (requestType) {
                    ...
                    case RequestInfo.TYPE_WEATHER_BY_WEATHER_LOCATION_REQ:
                        ...
                        if (status == MoKeeWeatherManager.RequestStatus.COMPLETED) {
                            ...
                            if (weatherInfo == null) {
                                ...
                                status = MoKeeWeatherManager.RequestStatus.FAILED;
                            } else {
    ...
    private boolean isValidRequestInfoStatus(int state) {
        switch (state) {
            case MoKeeWeatherManager.RequestStatus.COMPLETED:
            case MoKeeWeatherManager.RequestStatus.ALREADY_IN_PROGRESS:
            case MoKeeWeatherManager.RequestStatus.FAILED:
            case MoKeeWeatherManager.RequestStatus.NO_MATCH_FOUND:
    ...
    private final IBinder mService = new IMoKeeWeatherManager.Stub() {
        ...
        public String getActiveWeatherServiceProviderLabel() {
            ...
            final long identity = Binder.clearCallingIdentity();
            try {
                String enabledProviderService = MoKeeSettings.Secure.getString(
                        mContext.getContentResolver(), MoKeeSettings.Secure.WEATHER_PROVIDER_SERVICE);
    ...
    public MoKeeWeatherManagerService(Context context) {
        super(context);
        mContext = context;
    }
    ...
    public String getFeatureDeclaration() {
        return MoKeeContextConstants.Features.WEATHER_SERVICES;
    }

    @Override
    public void onStart() {
        publishBinderService(MoKeeContextConstants.MK_WEATHER_SERVICE, mService);
    ...
    private void bindActiveWeatherProviderService() {
        String activeProviderService = MoKeeSettings.Secure.getString(mContext.getContentResolver(),
            ...
            if (listener != null && listener.asBinder().pingBinder()) {
                try {
                    listener.onWeatherRequestCompleted(info,
                            MoKeeWeatherManager.RequestStatus.FAILED, null);
                } catch (RemoteException e) {
                }
            }
            return false;
            ...
            public void onPackageModified(String packageName) {
                ...
                if (cn.getPackageName().equals(packageName) && !mIsWeatherProviderServiceBound) {
                    ...
                    if (!getContext().bindServiceAsUser(new Intent().setComponent(cn),
                            mWeatherServiceProviderConnection, Context.BIND_AUTO_CREATE,
                            UserHandle.CURRENT)) {
                        MoKeeSettings.Secure.putStringForUser( mContext.getContentResolver(),
                                MoKeeSettings.Secure.WEATHER_PROVIDER_SERVICE, null,
                                getChangingUserId());
    ...
    private void registerSettingsObserver() {
        final Uri enabledWeatherProviderServiceUri = MoKeeSettings.Secure.getUriFor(
                MoKeeSettings.Secure.WEATHER_PROVIDER_SERVICE);
        ContentObserver observer = new ContentObserver(BackgroundThread.getHandler()) {
            @Override
            public void onChange(boolean selfChange, Uri uri, int userId) {
                if (enabledWeatherProviderServiceUri.equals(uri)) {
                    String activeSrvc = MoKeeSettings.Secure.getString(mContext.getContentResolver(),
                            MoKeeSettings.Secure.WEATHER_PROVIDER_SERVICE);
                    disconnectClient();
                    if (activeSrvc != null) {
